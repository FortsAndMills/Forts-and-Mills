...
QVector <QString> HelpManager::HelpInfo(QString name, GameRules * rules)
{
    ...
    {
        QVector<QString> ans;
        ans << "<u>Рекрут</u> - рекрутирует в клетке нового юнита выбранного типа. Клетка должна быть захвачена."
            << "Новый юнит появится в клетке в конце раунда. Враг не должен войти в клетку до конца раунда."
            << "В каждой клетке каждый игрок может создать за игру не более одного юнита.";
    ...
    if (name == "Fire") return QVector<QString>()
                ...
                << "При попадании учитываются все штрафы и бонусы юнитов и приказов, которые они исполняли (бонусы в обороне, штрафы приказов).";
...
//        }
    ...
    {
        QVector<QString> ans;
        ans << "<u>Щит</u>. Даёт единоразовый бонус в обороне +1. Этот бонус учитывается до бонуса форта и бонусов юнита.";

        if (!rules->ordersInGame.contains("Liberate"))
            ans << "Если противник входит в незащищённую юнитами клетку, все щиты на ней исчезают.";
        else
            ans << "При освобождении клетки все щиты противника в ней сгорают.";
        return ans;
    }
